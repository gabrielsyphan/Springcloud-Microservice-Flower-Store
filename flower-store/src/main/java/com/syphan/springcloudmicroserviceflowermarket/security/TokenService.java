    ...
    public String createJwtToken(UserEntity user) {
        try {
            ...
            String token = JWT.create()
                    .withIssuer("API - SpringCloud Microservice Store")
                    .withSubject(user.getEmail())
                    .withClaim("id", user.getId()) // Method used to add data to token payload
                    .withExpiresAt(this.getExpirationDate())
                    .sign(algorithm);
            return token;
        } catch (JWTCreationException exception) {
            throw new RuntimeException("Error to create jwt token");
        }
    }
